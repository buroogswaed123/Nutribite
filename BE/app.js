...
// ========================
// Simple password reset flow for FE PasswordReset.jsx
// ========================
app.post('/api/password_reset_simple', async (req, res) => {
  try {
    ...
    const userRows = await db.promise().query(
      'SELECT user_id, email FROM users WHERE email = ? OR username = ? LIMIT 1',
    ...
    // 1.5) Ensure new password differs from current
    try {
      const [[u]] = await db.promise().query('SELECT password_hash FROM users WHERE user_id = ? LIMIT 1', [user_id]);
      ...
      // If compare fails, proceed but log
      console.warn('Compare new vs old password failed:', e?.message || e);
    }

    // 2) Insert into password_resets the plaintext (per current table design)
    try {
      ...
      // If table shape is different, report clearly
      return res.status(500).json({ message: 'Failed to write password_resets entry', error: e.message });
    }

    // 3) Hash and update users.password_hash
    const hashed = await bcrypt.hash(newPassword, 10);
    await db.promise().query('UPDATE users SET password_hash = ? WHERE user_id = ?', [hashed, user_id]);

    return res.json({ success: true });
  } catch (err) {
    console.error('password_reset_simple error:', err);
